...
def covariance_loss(x: Tensor) -> Tensor:
    ...
    x = x - x.mean(dim=0) # 首先，对输入的张量 x 按照第一个维度（batch 维度）计算平均值，然后将每个样本减去对应维度上的平均值，这样做可以使得数据的均值为 0。
    batch_size = x.size(0) # 获取批大小。
    dim = x.size(-1)  # 获取特征维度
    # nondiag_mask has shape (dim, dim) with 1s on all non-diagonal entries.
    # 创建一个与协方差矩阵形状相同的掩码，其中对角线元素为 False，非对角线元素为 True。这个掩码用于过滤出协方差矩阵的非对角线元素。
    nondiag_mask = ~torch.eye(dim, device=x.device, dtype=torch.bool)
    # cov has shape (..., dim, dim)
    # 使用 Einstein 求和约定计算协方差矩阵。这里利用了 PyTorch 的 einsum 函数，对每个样本的特征进行组合，然后除以自由度(batch_size - 1)得到协方差矩阵。
    cov = torch.einsum("b...c,b...d->...cd", x, x) / (batch_size - 1)
    # 最后，通过对协方差矩阵应用掩码、平方和求和来计算协方差损失。
    loss = cov[..., nondiag_mask].pow(2).sum(-1) / dim
    return loss.mean()  # 返回损失的平均值作为最终结果。
